...
serve(async (req) => {
  ...
  try {
    ...
    // Only fetch runs whose scheduled_at <= nowWithBuffer (respects time-based scheduling)
    ...
    for (const run of allEngagementRuns) {
      ...
      // 3. Check for RECENTLY FAILED runs with "Active Order" errors (Cooldown Logic)
